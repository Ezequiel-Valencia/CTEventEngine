...
class Cafe9Scraper(Scraper):
    ...
    def retrieve_from_source(
        ...
        return [
            AllEventsFromAGroup(events, Cafe9Scraper.group_kernel, Cafe9Scraper.url)
        ]
    ...
    def get_pages_content(self, href) -> GenericEvent:
            ...
            if i == 0:
                ...
                for time_format in potential_formats:
                    try:
                        ...
                        logger.warn(msg="Getting time did not work.", exc_info=err)

                if time is None:
                    raise ValueError("Could not gather start time for event.")
                event.begins_on = (
                    pytz.timezone("America/New_York").localize(time).isoformat()
                )
        ...
        event.physical_address = GenericAddress(
            geom="-72.9238838019238;41.30377450753123",
            locality="New Haven",
            postalCode="06510",
            street="250 State St",
            region="CT",
